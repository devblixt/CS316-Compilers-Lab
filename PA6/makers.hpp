		...
		for (CodeLine *line : code->threeAC)
		{
			...
			else
			{
				...
				for (std::string i : Ids)
				{
					if (i == com.substr(0, 3))
					{
						assembly.push_back(CodeLine(line->scope, "move", getInterRegister(farg1), getInterRegister(farg3)));

						assembly.push_back(CodeLine(line->scope, getLower(com), getInterRegister(farg2), getInterRegister(farg3)));
